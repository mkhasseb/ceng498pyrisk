...
COLOR_ORANGE = "Orange"
COLOR_YELLOW = "Yellow"
COLOR_EMPTY = "unoccupied"
clientHelpers = {}
mapFileNames = {}
occupantColors = { COLOR_RED : "#FD121A",  COLOR_LPINK : "#EE8FFF", COLOR_GREEN : "#93FF1F", COLOR_GRAY: "#B7B7B7", COLOR_ORANGE: "#FB710E", COLOR_YELLOW:"#F1ED04", COLOR_EMPTY: "#FFFFFF"}
continentColors = ["#029D11", "#010AFE", "#9703D6", "#713800", "#601515", "#000000", "#FFFFFF"]

MY_ROOT = os.path.join(os.path.dirname(__file__), '.')
...
@get('/static')
def my_media(req):
    filename = req.GET.get('filename', None)
...
def mapFileName(req):
    try:
        ...
        if(not id or not id in clientHelpers):
            ...
            return "http://localhost:10000/static?filename=" + clientHelpers[id].fname
    except Exception as e:
        traceback.print_exc(file=sys.stdout)
    ...
    try:
        ...
        if(not id or not id in clientHelpers):
            ...
            for region in regions:
                ...
                r={'name':name, 'occupant':occupantColors[color], 'armyNum':armyNum}
...
def mission(req):
    try:
        ...
        if(not id or not id in clientHelpers):
            ...
            return clientHelpers[id].mission
    except Exception as e:
        traceback.print_exc(file=sys.stdout)
        return 'Error %s, parameters were id=%s' % (e, id)
...
def playerName(req):
    try:
        ...
        if(not id or not id in clientHelpers):
            ...
            return occupantColors[clientHelpers[id].playername]
    except Exception as e:
        traceback.print_exc(file=sys.stdout)
        return 'Error %s, parameters were id=%s' % (e, id)
